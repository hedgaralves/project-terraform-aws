...
resource "aws_lambda_function" "getLambda1" {
    function_name = "getLambda1"
    s3_bucket = "project-terraform-live"
    s3_key = "getPerson1.zip"
    role = aws_iam_role.getRole.arn
    handler = "getPerson1.handler"
    runtime = "nodejs14.x"
}
...
resource "aws_api_gateway_integration" "writeInt" {
    ...
    type = "AWS_PROXY"
    uri = aws_lambda_function.registerLambda1.invoke_arn
...
resource "aws_api_gateway_integration" "readInt" {
    ...
    integration_http_method = "POST"
    type = "AWS_PROXY"
    uri = aws_lambda_function.getLambda1.invoke_arn
...
resource "aws_lambda_permission" "writePermission" {
    ...
    function_name = aws_lambda_function.registerLambda1.function_name
...
resource "aws_lambda_permission" "readPermission" {
    statement_id  = "AllowExecutionFromAPIGateway"
    action = "lambda:InvokeFunction"
    function_name = aws_lambda_function.getLambda1.function_name
    principal = "apigateway.amazonaws.com"
